...
import type { Key, MenuData, ProLayoutProps } from './typing'
...
function layoutStateFunc(props: ProLayoutProps, methods: ProLayoutProviderMethods = {}) {
  ...
  const menuDataMap = reactive(new Map<Key, API.MenuVO>())
  ...
  watch(
    ...
    () => {
      ...
      menuData.value?.forEach((item) => {
        menuDataMap.set(item.path as string, item)
      })
    },
    {
      immediate: true,
  ...
  const selectedMenus = computed(() => {
    if (isMobile.value || layout.value !== ('mix' as string) || !splitMenus.value)
    ...
    () => {
      if (splitMenus.value) {
        ...
        if (key) {
          ...
          if (find) splitState.selectedKeys = [find.path as string]
        }
      }
    },
    {
