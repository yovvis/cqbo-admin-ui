...
import { useLayoutState } from '../basic/context.ts'
...
const siderStyle = computed<CSSProperties>(() => {
  ...
  if (layout.value === ('mix' as string) && header.value === false) style.paddingTop = '0px'
...
const showLogo = computed(() => {
  return (layout.value === 'side' || isMobile.value) && layout.value !== ('mix' as string)
})
</script>
...
<template>
  ...
  <a-layout-sider
    ...
    :theme="theme === ('inverted' as string) ? 'dark' : 'light'"
  ...
  >
    ...
    <div
      ...
      :class="theme === ('inverted' as string) ? 'ant-pro-sider-collapsed-button-inverted' : ''"
    >
      <a-menu
