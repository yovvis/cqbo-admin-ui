        ...
        <a-dropdown :trigger="['contextmenu']">
          ...
          <template #overlay>
            <a-menu @click="handleSwitch($event, item.fullPath)">
              ...
              <a-menu-item key="refresh" :disabled="!isCurrentDisabled">刷新当前 </a-menu-item>
            </a-menu>
          </template>
        </a-dropdown>
      </template>
        ...
        <a-dropdown :trigger="['hover']">
          ...
          <template #overlay>
            <a-menu @click="handleSwitch($event, activeKey)">
              <a-menu-item key="closeOther" :disabled="isCurrentDisabled || otherDisabled">
                <!-- 关闭其他 -->
                {{ 'app.multiTab.closeOther' }}
              </a-menu-item>
              <a-menu-item key="refresh">
                <!-- 刷新当前 -->
                {{ 'app.multiTab.refresh' }}
              </a-menu-item>
            </a-menu>
          </template>
        </a-dropdown>
...
import { useLayoutState } from '@/layouts/basic/context.ts'
...
const tabStyle = computed<CSSProperties>(() => {
  ...
  if (
    layoutSetting.value.header === false ||
    (layout.value !== ('mix' as string) && layoutSetting.value.fixedHeader === false)
  )
    style.top = '0px'
